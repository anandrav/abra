...
use exhaustive::{check_pattern_exhaustiveness_toplevel, DeconstructedPat};
...
impl TypeVar {
    ...
    fn from_node(ctx: &mut StaticsContext, id: NodeId) -> TypeVar {
        let prov = Prov::Node(id);
        Self::fresh(ctx, prov)
    }

    fn fresh(ctx: &mut StaticsContext, prov: Prov) -> TypeVar {
...
fn ast_type_to_statics_type(ctx: &mut StaticsContext, ast_type: Rc<AstType>) -> TypeVar {
...
fn gather_definitions_toplevel(ctx: &mut StaticsContext, gamma: Gamma, toplevel: Rc<Toplevel>) {
...
fn generate_constraints_toplevel(gamma: Gamma, toplevel: Rc<Toplevel>, ctx: &mut StaticsContext) {
    for statement in toplevel.statements.iter() {
...
fn result_of_constraint_solving(
    ctx: &mut StaticsContext,
    node_map: &NodeMap,
    sources: &Sources,
) -> Result<(), String> {
...
fn result_of_additional_analysis(
    ctx: &mut StaticsContext,
