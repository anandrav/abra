    ...
    fn instantiate_iface_output_types(
        ...
        let ty = match ty {
            ...
            PotentialType::InterfaceOutput(_, ref output_type) => {
                ...
                for constraint in ifaces {
                    println!("constraint name = {}", constraint.iface.name.v);
                    println!("nargs = {}", constraint.args.len());
                    ret.0.with_data(|d| {
                        d.iface_constraints
                            .entry(constraint)
                            .or_default()
    ...
    for i in ast_ifaces {
        ...
        for (name, val) in &i.arguments {
            ...
            let Some(val) = ast_type_to_solved_type(ctx, val.clone()) else {
                panic!(); // TODO: should report an error here?
                continue;
            };
...
fn tyvar_of_iface_method(
    ...
    if let Some(desired_impl_ty) = desired_impl_ty
        && let Some(impl_list) = ctx.interface_impls.get(&iface_def)
