    ...
    pratt
        ...
        .map_postfix(|lhs, op| match op.as_rule() {
            ...
            Rule::member_func_call => {
                ...
                Rc::new(Expr {
                    kind: Rc::new(ExprKind::MemberFuncAp(Some(lhs), fname, args)),
                    loc: span,
                    id: NodeId::new(),
                })
            }
...
pub(crate) fn parse_type_poly(pair: Pair<Rule>, file_id: FileId) -> Rc<Polytype> {
    ...
    match rule {
        Rule::type_poly => {
            let inner: Vec<_> = pair.into_inner().collect();
            let mut ty_name = inner[0].as_str().to_owned();
            if ty_name.starts_with('\'') {
                ty_name = ty_name[1..].to_owned();
            }
...
pub(crate) fn parse_item(pair: Pair<Rule>, file_id: FileId) -> Rc<Item> {
    ...
    match rule {
        ...
        Rule::func_decl => Rc::new(Item {
            kind: Rc::new(ItemKind::FuncDecl(parse_func_decl(inner, file_id))),
            loc: span,
            id: NodeId::new(),
        }),
        // Rule::typealias => {
        ...
        Rule::interface_declaration => {
            ...
            for pair in inner.iter().skip(1) {
                ...
                match rule {
                    Rule::func_decl => {
                        let func_decl = parse_func_decl(inner, file_id);
                        props.push(func_decl);
                    }
                    Rule::output_type_decl => {
                        ...
                        let outputtype = InterfaceOutputType {
                            name,
                            interfaces,
                            id: NodeId::new(),
                        };
...
fn parse_func_decl(pairs: Vec<Pair<'_, Rule>>, file_id: FileId) -> Rc<FuncDecl> {
    let mut n = 0;
    ...
    let mut args = vec![];
    let name = Identifier {
        v: pairs[n].as_str().to_string(),
        loc: Location::new(file_id, pairs[n].as_span()),
        id: NodeId::new(),
    }
    .into();
    n += 1;
    while let Rule::func_arg_annotated = pairs[n].as_rule() {
        let pat_annotated = parse_func_arg_annotation_mandatory(pairs[n].clone(), file_id);
        args.push(pat_annotated);
        n += 1;
    }

    let maybe_func_out = &pairs[n];
...
pub(crate) fn parse_stmt(pair: Pair<Rule>, file_id: FileId) -> Rc<Stmt> {
    ...
    match rule {
        ...
        Rule::while_statement => {
            ...
            Rc::new(Stmt {
                kind: Rc::new(StmtKind::WhileLoop(cond, statements)),
                loc: span,
                id: NodeId::new(),
            })
        }
...
pub(crate) fn parse_expr_term(pair: Pair<Rule>, file_id: FileId) -> Rc<Expr> {
    ...
    match rule {
        ...
        Rule::enum_constructor_inferred => {
            let inner: Vec<_> = pair.into_inner().collect();
            let fname = Identifier {
                v: inner[0].as_str().to_string(),
                loc: Location::new(file_id, inner[0].as_span()),
            ...
            Rc::new(Expr {
                kind: Rc::new(ExprKind::MemberFuncAp(None, fname, args)),
                loc: span,
                id: NodeId::new(),
            })
        }
