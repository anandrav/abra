...
"#;
    ...
    runtime.run();
    let top = runtime.top();
    assert_eq!(top.get_int(runtime.main()), 4);
}

#[test]
fn import_from_subdir() {
    let util = r#"
fn foo(a: int, b) {
  a + b
}
...
"#;
    let mut files: HashMap<PathBuf, String> = HashMap::default();
    files.insert("main.abra".into(), main.into());
    files.insert(PathBuf::from("more").join("util.abra"), util.into());
    let file_provider = MockFileProvider::new(files);
...
"#;
    ...
    runtime.run();
    let top = runtime.top();
    assert_eq!(top.get_int(runtime.main()), 42);
}
...
"#;
    ...
    runtime.run();
    let top = runtime.top();
    assert_eq!(top.get_int(runtime.main()), 84);
}
...
"#;
    ...
    runtime.run();
    let top = runtime.top();
    assert_eq!(top.get_int(runtime.main()), 204);
}
