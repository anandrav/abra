...
impl ExpectedValue for AbraInt {
    fn check(self, runtime: &mut Runtime, val: Value) {
        assert_eq!(val.get_int(&runtime.main_thread), self);
    }
}

impl ExpectedValue for i32 {
    fn check(self, runtime: &mut Runtime, val: Value) {
        assert_eq!(val.get_int(&runtime.main_thread), self as AbraInt);
    }
}

impl ExpectedValue for AbraFloat {
    fn check(self, runtime: &mut Runtime, val: Value) {
        assert_eq!(val.get_float(&runtime.main_thread), self);
    }
}

impl ExpectedValue for f32 {
    fn check(self, runtime: &mut Runtime, val: Value) {
        assert_eq!(val.get_float(&runtime.main_thread), self as AbraFloat);
    }
}

impl ExpectedValue for bool {
    fn check(self, runtime: &mut Runtime, val: Value) {
        assert_eq!(val.get_bool(&runtime.main_thread), self);
    }
}

impl ExpectedValue for &str {
    fn check(self, runtime: &mut Runtime, val: Value) {
        assert_eq!(val.view_string(&runtime.main_thread), self);
    }
}
